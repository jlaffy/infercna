...
#' @param expr.center.rows logical value specifying whether input expression matrix should be row-centered. Default: TRUE
#' @param cna.center.cols logical value specifying whether computed CNA matrix should be column-centered. Default: TRUE
#' @param cna.center.method method by which to center the cells after calculating CNA values. One of 'median', 'mean', etc.... Default: 'median'
#' @param isLogApplicable boolean value indicating whether the input expression matrix <m> is or can be log2-normalised. Set to FALSE if an alternative normalisation of the expression data was already performed. Default: TRUE
...
#' @rdname infercna3
#' @export 
infercna3 = function(m,
                    refCells = NULL,
                    window = 100,
                    range = c(-3, 3),
                    n = NULL,
                    noise = 0.1,
                    expr.center.rows = TRUE,
                    cna.center.cols = TRUE,
                    cna.center.method = 'median',
		            window.break = c('chr', 'arm'),
		            exclude.chr = NULL, #c('Y','X')
                    isLogApplicable = TRUE,
                    isLog = TRUE,
                    verbose = TRUE,
                    genome = c('hg19', 'hg38')) {
    ...
    # note: m should NOT be row(gene)-centered if expr.center.rows == TRUE
    if (expr.center.rows && all(round(range(rowMeans(m)), 3) == 0)) {
        stop('Matrix is row-centered. Please provide non-centered data or set expr.center.rows to FALSE.')
    }


    if (!is.null(n)) {

    	if (isLogApplicable && isLog) {
    	    if (verbose) {
    ...
    if (verbose) {
        if (expr.center.rows) {
            ...
            message('Skipping mean-centering of the genes...')
        }
    }
    ...
    if (verbose) {
	    message('Performing ', cna.center.method, '-centering of the cells...')
    }
    
    cna = colcenter(cna, by = cna.center.method) # note: using median centering here
