...
## Data: a patient BT771 with Glioblastoma

To explore the basic functionality of infercna, we'll use `infercna::bt771`. This dataset contains `r ncol(infercna::bt771)` cell columns, each of which is described with RNA-sequencing data of `r nrow(infercna::bt771)` gene variables. The data comes from a large study that we did looking at 28 patients with Glioblastoma (a very aggressive form of brain cancer). You can look at the publication [here](Neftel*, Laffy* et al. 2019, Cell) and the data is easily available via the [Single Cell Portal](https://portals.broadinstitute.org/single_cell/study/SCP393/single-cell-rna-seq-of-adult-and-pediatric-glioblastoma). `bt771` is one of the 28 patients and this much smaller dataset is documented in `?bt771`.
...
Note that `infercna::bt771` is a tibble, a modern reimagining of the data frame. It's particularly useful for large datasets because it only prints the first few rows. You can learn more about tibbles at <http://tibble.tidyverse.org>; in particular you can convert data frames to tibbles with `as_tibble()`.
...
* `toAbsolute()` to select cases based on their values.
...
* option to calculate absolute CNAs (see `toAbsolute()`) with just *one* reference group. Currently infercna requires at least two, which, while preferable, is not obligatory.
